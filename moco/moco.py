...
class MoCo(nn.Module): 
    ...
    def forward_aug_nn(self, x1, x2, y):
        q = self.encoder_q.get_embedding2(x1)
        latent = self.encoder_q.get_embedding1(x1)
        latent_classifier = self.encoder_q.get_embedding3(x1)
        ...
        with torch.no_grad():
            ...
            k1 = self.encoder_k.get_embedding2(x1)
            k2 = self.encoder_k.get_embedding2(x2)
        ...
        # class_embeddings = [latent[y == label] for label in unique_labels]
        # mean_embeddings = torch.cat([class_emb.mean(dim=0, keepdim=True) for class_emb in class_embeddings])
        # selected_mean_embeddings = torch.nn.functional.embedding(y, mean_embeddings)
        # # selected_mean_embeddings = mean_embeddings[y]
        # regularization_loss = F.mse_loss(latent, selected_mean_embeddings)
        # loss+=10*regularization_loss
        self._dequeue_and_enqueue(k2)

        return loss, latent_classifier
    ...
    #     return loss, latent
    def forward(self, x1, x2, y, flag="aug_nn"):
        if flag == 'aug_nn':
            return self.forward_aug_nn(x1, x2, y)
    ...
    def get_embedding(self, x):
        ...
        # out = self.encoder_q.get_embedding1(x)
        out = self.encoder_q.get_embedding2(x)
        return out
    
    def get_embedding_pred(self,x):
        out = self.encoder_q.get_embedding3(x)
        return out